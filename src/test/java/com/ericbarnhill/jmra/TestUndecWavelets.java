...
public class TestUndecWavelets {
    ...
    public void testUndecWaveletRecon() {
        ...
        //DEBUGGING
        ArrayMath.displaySize(image);
        MRA2DU mra = new MRA2DU(image, filterBank, 1, ConvolverFactory.ConvolutionType.FDCPU);
        mra.dwt();
...
//        mra.threshold(Threshold.ThreshMeth.SOFT, Threshold.NoiseEstMeth.VISU_SHRINK);
        ...
        for (int n = 0; n < decomp.size(); n++) {
            String path = root + Integer.toString(n)+ "_after.tif";
            array2Image(decomp.get(n), path);
        }
